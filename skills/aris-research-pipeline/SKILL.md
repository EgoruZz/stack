...
> **Local-only deployment note:** this installation intentionally has no Codex,
> Oracle, or manual-review MCP. Never call unavailable `mcp__codex__*` or
> `mcp__manual_review__*` tools. Use local OpenCode `Task` subagents for
> adversarial review and label the result as local review, not independent
> cross-model evidence. If a stage requires an unavailable external verdict,
> stop with `REVIEW_UNAVAILABLE` rather than fabricating a pass.
